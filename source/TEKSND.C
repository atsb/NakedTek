...
_SOS_CAPABILITIES *digicapptr=( _SOS_CAPABILITIES *)&digicap;
...
_SOS_HARDWARE  *digihardwareptr=( _SOS_HARDWARE *)&digihardware;
...
_SOS_INIT_DRIVER  *digiinitdriverptr=( _SOS_INIT_DRIVER *)&digiinitdriver;
...
_SOS_START_SAMPLE   *sampleptr[MAXSOUNDS];
...
VOID      digiloopcallback(  WORD, WORD, WORD  );
...
_SOS_START_SAMPLE   *loopsampleptr[MAXLOOPS];
...
WORD      digiport;
WORD *digiportptr=( WORD *)&digiport;
WORD      fhdigifill;
WORD *fhdigifillptr=( WORD *)&fhdigifill;
WORD      fhdigidriver;
WORD *fhdigidriverptr=( WORD *)&fhdigidriver;
...
_SOS_MIDI_CAPABILITIES *midicapptr=( _SOS_MIDI_CAPABILITIES *)&midicap;
...
_SOS_MIDI_DIGI_INIT_DRIVER    *mididigiinitdriverptr=(_SOS_MIDI_DIGI_INIT_DRIVER *)&mididigiinitdriver;
...
_SOS_MIDI_INIT_DRIVER    *midiinitdriverptr=( _SOS_MIDI_INIT_DRIVER*)&midiinitdriver;
...
_SOS_MIDI_INIT_SONG *songdataptr=(_SOS_MIDI_INIT_SONG*)&songdatastatic;
...
_SOS_MIDI_HARDWARE  *midihardwareptr=( _SOS_MIDI_HARDWARE *)&midihardware;

WORD      fhmididriver;
WORD *fhmididriverptr=( WORD *)&fhmididriver;
WORD      fhmididigidriver;
WORD *fhmididigidriverptr=( WORD *)&fhmididigidriver;
...
_SOS_MIDI_TRACK_DEVICE   *trackmapptr=(_SOS_MIDI_TRACK_DEVICE *)&songtrackmap;
...
VOID      soundcallback(  WORD, WORD, WORD  );
void      timerhandler(void);
unsigned  timerhandle1;
...
initaudioptrs()
{
     ...
     for( i=0; i<MAXSOUNDS; i++ ) {
          ...
          sampleptr[i]=(_SOS_START_SAMPLE*)&sample[i];
...
//jsa venom
     ...
     for( i=0; i<MAXLOOPS; i++ ) {
          ...
          loopsampleptr[i]=(_SOS_START_SAMPLE*)&loopsampledata[i];
     }
}
...
initsb(char option1,char option2,long digihz,char option7a,char option7b,int val,char option7c)
{
     ...
     if( musicmode != MM_NOHARDWARE ) {
          ...
          midiinitdriverptr->lpDriverMemory=( VOID *)_NULL;
          midiinitdriverptr->sDIGIInitInfo=( _SOS_MIDI_DIGI_INIT_DRIVER *)NULL;
...
playsong(int sn)
{
     ...
     songdataptr->lpSongCallback=( VOID *)NULL; //songcallback;
     ...
     if( songptr[sn]->handle == NULL_HANDLE ) {
          ...
          rv=sosMIDIInitSong(songdataptr, trackmapptr, ( WORD *)&(songptr[sn]->handle));
          if( rv != _ERR_NO_ERROR ) {
               songptr[sn]->handle=NULL_HANDLE;
               return(0);
          }
